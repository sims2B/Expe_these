...
//---------------------------------------------------------------------------------------------------
IloNumVarArray t; 
/*IloIntVarMatrix x, y;//*/IloNumVarMatrix x, y;
IloNumVarMatrix b;
...
void Event2() {
	...
	define_data(env);
        ...
        /*x = IloIntVarMatrix(env,N); //*/ x = IloNumVarMatrix(env,N);
        /*y = IloIntVarMatrix(env,N); //*/ y = IloNumVarMatrix(env,N);

        for (e=0;e<n;e++) b[e] = IloNumVarArray(env,K, 0, infini);

        for (i=0;i<N;i++){     
           /*x[i] = IloIntVarArray(env,n,0,1); //*/ x[i] = IloNumVarArray(env,n,0,1);
           /*y[i] = IloIntVarArray(env,n,0,1); //*/ y[i] = IloNumVarArray(env,n,0,1); /// A REVOIR
        };
	...
	//*
	   ...
	   for(f=e;f<n;f++)//ici
		for(i=0;i<N;i++) 
		  model.add(t[f] >= t[e] + p[i] * x[i][e] - p[i] * (1-y[i][f]) );
...
//*/
	...
	strncpy(ResultFileName, ResultFile.c_str(), 100 );
	std::ofstream SolOuv(ResultFileName, std::ios_base::app );
        if(!SolOuv){
