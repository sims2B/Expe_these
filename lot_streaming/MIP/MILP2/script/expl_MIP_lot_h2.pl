...
sub handle_file {
    ...
    if ($file=~ /.*$/) {
	    ...
	    system("timeout --signal=9 7200s ./bin/modelLotSE_h2 $file 2 2>&1 >> $target2; echo \"\$?\" >>$target2");
	    system("timeout --signal=9 7200s ./bin/modelLotSE_h2 $file 3 2>&1 >> $target3; echo \"\$?\" >>$target3");
	    system("timeout --signal=9 7200s ./bin/modelLotSE_h2 $file 4 2>&1 >> $target4; echo \"\$?\" >>$target4");
