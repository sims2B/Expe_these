...
int main( int argc, char* argv[]){
  ...
  while (cpt < argc-1){ 
	...
	if (inst_name == cell_vec[i].name){
	  if (cell_vec[i].best==0 && res_line.status==3)  cell_vec[i].best = res_line.final_obj ;
	  else if (cell_vec[i].best==0 && res_line.status==2)    cell_vec[i].best = res_line.final_obj  - res_line.final_gap*( 1*pow(10,-10) + res_line.final_obj );
	  else if (res_line.final_obj < cell_vec[i].best && res_line.final_obj!=0 && res_line.status==3)
	    cell_vec[i].best = res_line.final_obj ;
	  else if (res_line.final_obj < cell_vec[i].best && res_line.final_obj!=0 && res_line.status==2)
	    cell_vec[i].best = res_line.final_obj  - res_line.final_gap*( 1*pow(10,-10) + res_line.final_obj );
	}
      }
