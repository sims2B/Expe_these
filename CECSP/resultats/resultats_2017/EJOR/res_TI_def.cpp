...
res::res() : size(0), time_first_sol(0.0), gap_first_sol(0.0), time_end(0.0), gap_end(0.0), trouve (0) , solved(0),opt(0),obj(0.0), best_obj(0.0),ecart(0.0){}
...
int main( int argc, char* argv[]){
  ...
  for (int j=0;j<5;++j){
	...
	if (res_vec[i].trouve){
	  if (res_vec[i].best_obj !=0) res_line.ecart+= sqrt(pow(res_vec[i].best_obj - res_vec[i].obj,2)/2.0); 
	  ++nb_trouv;
	  if (res_vec[i].solved ){
	    res_line.gap_first_sol+=res_vec[i].gap_first_sol;
	...
	if (res_vec[i].trouve)
	  res_line.opt+=res_vec[i].opt;
	if (res_vec[i].trouve){
	  if (res_vec[i].best_obj !=0)
	    res_line.ecart+= sqrt(pow(res_vec[i].best_obj - res_vec[i].obj,2)/2.0); 
	  ++nb_trouv;
	  if (res_vec[i].solved ){
	    res_line.gap_first_sol+=res_vec[i].gap_first_sol;
	    res_line.time_first_sol+=res_vec[i].time_first_sol;
	...
	res_line.solved+=res_vec[i].solved;
	res_line.time_end+= res_vec[i].time_end;
	if (res_vec[i].trouve){if (res_vec[i].best_obj !=0)
	    res_line.ecart+= sqrt(pow(res_vec[i].best_obj - res_vec[i].obj,2)/2.0); 
	  res_line.opt+=res_vec[i].opt;
	  ++nb_trouv;
	  if (res_vec[i].solved ){
    ...
    if( nb_trouv!= 0){
      res_line.ecart=res_line.ecart/nb_trouv;
